...
class Command(BaseCommand):
    help = f'Send email changes for {DAYS_FOR_SENDCHANGES} days in project'

    def handle(self, *args, **options):
        logger.debug(f'email will be send Command->handle')
        ...
        logger.debug(f'Command->handle {prj_groups=}')
        ...
        for project in active_projects:
            ...
            logger.debug(f'mail_content:{project.slug=} _ {mail_content[project.slug]}')
            ...
            if emails and mail_content[project.slug]:
                try:
                    ...
                    logger.debug(f"About to send email to {emails}")
                    # Logic to send email here
                    for email_to_send in emails:
                        send_mail(
                                f'B2B [{project.title}] updates digest {datetime.now().date().strftime("%d %B, %Y")}',
                                mail_content[project.slug].replace(u'\xa0', u' '),  # \xa0 non-breaking space
                                settings.EMAIL_HOST_USER,
                                [email_to_send],
                                fail_silently=False,
                        )
                    logger.debug("After successful sent email ")
    ...
    def select_last_topics(self, project_page, days=DAYS_FOR_SENDCHANGES) -> str:
        ...
        logger.debug(f'today: {d1}  time-delta: {d0} _ days: {days}')
    ...
    def get_subscriber_emails(self, project_page: Project) -> list:
        logger.debug(f'Create emails list for {project_page.title=}')
        try:
            selected_group = Group.objects.get(name=project_page.slug)
        except Group.DoesNotExist:
            logger.warning(f'Group does not exist. Creating new group {project_page.slug}')
        ...
        if project_users:
            ...
            logger.debug(f'Created emails list for {project_page.title=}')
        else:
            logger.warning(f'No users in the group {project_page.slug}. Skip creating users list.')
